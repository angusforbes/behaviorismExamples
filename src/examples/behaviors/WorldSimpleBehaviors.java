...
//    b1.attachGeom(g);
    Scheduler.getInstance().attachGeom(b1, g);
    ...
    //b2.attachGeom(g);
    Scheduler.getInstance().attachGeom(b2, g);
  ...
  public void testChangeSpeed()
  {
    ...
    BehaviorTranslate bd = new BehaviorTranslate(
      new ContinuousBehaviorBuilder(base, 1000L).isReversing(true).ranges(new float[]{2f,0f,0f}).repeats(5));
    //bd.attachGeom(g);
    Scheduler.getInstance().attachGeom(bd, g);

    BehaviorSpeed bs = new BehaviorSpeed(Utils.nanoPlusMillis(base, 500L), 2f);
    Scheduler.getInstance().attachBehavior(bs, bd);

    BehaviorSpeed bs2 = new BehaviorSpeed(Utils.nanoPlusMillis(base, 1500L), 1f/6f);
    Scheduler.getInstance().attachBehavior(bs2, bd);

    BehaviorSpeed bs3 = new BehaviorSpeed(Utils.nanoPlusMillis(base, 2500L), 10f);
    Scheduler.getInstance().attachBehavior(bs3, bd);


    Utils.sleep(2600);
 Scheduler.getInstance().interruptImmediately(bd);
    Utils.sleep(50);

    System.out.println("g behaviors size = " + g.behaviors.size());
    System.out.println("scheduler has # behaviors size = " + Scheduler.getInstance().behaviors.size());
  ...
  public void testSimpleRangeInterpolaters()
  {
    ...
    bc.isLooping = false;
    Scheduler.getInstance().attachGeom(bc, g);
    ...
    bc2.isLooping = false;
    //bc2.attachGeom(g2);
    Scheduler.getInstance().attachGeom(bc2, g);
...
//    bc.attachGeom(g2);
Scheduler.getInstance().attachGeom(bc, g2);

    BehaviorToggleSpeed bs = new BehaviorToggleSpeed(Utils.nowPlusMillis(5000L), 2000L);
    // Utils.sleep(1000);
Scheduler.getInstance().attachBehavior(bs, bc);
